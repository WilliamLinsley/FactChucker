package com.example.factchucker.domain.usecase.category
...
class GetJokeByCategoryUseCase @Inject constructor(
    ...
    operator fun invoke(category: String?): Flow<State<Joke>> = flow {
        try {
            emit(State.Loading())
            val joke = repository.getJokeByCategory(category).toJoke()
